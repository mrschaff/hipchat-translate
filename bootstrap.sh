...
apt-get install -y build-essential libssl-dev monit unzip vim curl ntp redis-server mongodb-org ngrok-client 3> /dev/null
...
check process ngrok matching "/usr/bin/ngrok"
    start program = "/bin/bash -c '2>&1 1>>/var/log/ngrok.log /usr/bin/ngrok -log=stdout 3000'"
...
sudo apt-get install --yes nodejs 2> /dev/null
