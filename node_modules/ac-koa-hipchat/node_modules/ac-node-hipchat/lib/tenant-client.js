...
TenantClient.prototype.getRoomMembers = function (roomIdOrName, options) {
  ...
  return self.getToken('view_room').then(function (token) {
...
TenantClient.prototype.glancePushUser = function (userId, options) {
  var self = this;
  exports.verifyNumberOrString('userId', userId);
  var resourceUrl = this._tenant.links.api + '/addon/ui/user/' + param(userId);
  return self.getToken('view_group').then(function (token) {
    return self._client.glancePushUser(resourceUrl, token, options);
  });
};

TenantClient.prototype.glancePushRoom = function (roomId, options) {
  var self = this;
  exports.verifyNumberOrString('roomId', roomId);
  var resourceUrl = this._tenant.links.api + '/addon/ui/room/' + param(roomId);
  return self.getToken('view_room').then(function (token) {
    return self._client.glancePushRoom(resourceUrl, token, options);
  });
};

TenantClient.prototype.glancePushGroup = function (options) {
  var self = this;
  var resourceUrl = this._tenant.links.api + '/addon/ui';
  return self.getToken('view_group').then(function (token) {
    return self._client.glancePushGroup(resourceUrl, token, options);
  });
};
